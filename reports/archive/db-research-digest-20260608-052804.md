...
- 生成时间：2026-06-08 05:28:04 UTC
- 跟踪渠道：arXiv cs.DB、PVLDB、PACMMOD、ICDE、CIDR、DBLP
- 本次纳入论文：84 篇
- 来源分布：arXiv cs.DB 35 篇；PVLDB 22 篇；PACMMOD 15 篇；CIDR 6 篇；ICDE 6 篇

## 本期最值得优先阅读

1. [Breaking the Isolation-Freshness Trade-off: Joint Adaptive Storage Optimization for HTAP Systems](https://www.vldb.org/pvldb/volumes/19/contributions)
...
2. [SpecDB: LLM-Generated Customized Databases via Feature-Oriented Decomposition](https://arxiv.org/pdf/2605.31097v1.pdf)
   - 来源：arXiv cs.DB | 日期：2026-05-29 | 评分：25 | 分类：存储引擎、查询优化、OLAP / 分析执行
   - 为什么值得看：有系统实现，带基准测试，带生产环境信号，关注延迟
   - 摘要判断：这篇工作主要落在 存储引擎、查询优化、OLAP / 分析执行。更偏系统实现。
3. [Sample-based Distinct Cardinality Estimation for Multiple Attributes in Multi-Dataset Queries](https://www.vldb.org/pvldb/volumes/19/contributions)
...
6. [Robust Predicate Transfer with Dynamic Execution](https://www.vldb.org/pvldb/volumes/19/contributions)
   - 来源：PVLDB | 日期：2026-02-01 | 评分：20 | 分类：查询优化、OLAP / 分析执行
   - 为什么值得看：有系统实现，带基准测试，涉及连接处理
   - 摘要判断：这篇工作主要落在 查询优化、OLAP / 分析执行。更偏系统实现。
7. [QDAG: Declarative Composition of Reusable Analytics Methodologies at LinkedIn](https://arxiv.org/pdf/2606.05662v1.pdf)
   - 来源：arXiv cs.DB | 日期：2026-06-04 | 评分：20 | 分类：查询优化、OLAP / 分析执行
   - 为什么值得看：有系统实现，带生产环境信号，关注延迟，涉及连接处理
   - 摘要判断：这篇工作主要落在 查询优化、OLAP / 分析执行。更偏系统实现。
8. [Efficient Methods for Accurate Sparse Trajectory Recovery and Map Matching.](https://arxiv.org/pdf/2508.10460)
...
- [Breaking the Isolation-Freshness Trade-off: Joint Adaptive Storage Optimization for HTAP Systems](https://www.vldb.org/pvldb/volumes/19/contributions)
  来源：PVLDB | 日期：2026-02-01 | 评分：29 | 作者：Zhenghao Ding, Xinyi Zhang, Chao Zhang, Yishen Sun
  为什么重要：有系统实现，带基准测试，有实验评估，贴近真实场景
  摘要判断：这篇工作主要落在 存储引擎、查询优化、OLAP / 分析执行。更偏系统实现。
- [SpecDB: LLM-Generated Customized Databases via Feature-Oriented Decomposition](https://arxiv.org/pdf/2605.31097v1.pdf)
  来源：arXiv cs.DB | 日期：2026-05-29 | 评分：25 | 作者：Yunkai Lou, Longbin Lai, Shunyang Li, Zhengping Qian
  为什么重要：有系统实现，带基准测试，带生产环境信号，关注延迟
  摘要判断：这篇工作主要落在 存储引擎、查询优化、OLAP / 分析执行。更偏系统实现。
...
- [SVFusion: A CPU-GPU Co-Processing Architecture for Large-Scale Real-Time Vector Search](https://www.vldb.org/pvldb/volumes/19/contributions)
  来源：PVLDB | 日期：2026-01-01 | 评分：18 | 作者：Yuchen Peng, Dingyu Yang, Zhongle Xie, Ji Sun
  为什么重要：有系统实现，关注吞吐，关注延迟
  摘要判断：这篇工作主要落在 存储引擎。更偏系统实现。
- [Terark-DS: A High-Performance and Storage-Efficient Key-Value Separation Storage Engine on Disaggregated Storage](https://www.vldb.org/pvldb/volumes/19/contributions)
  来源：PVLDB | 日期：2026-01-01 | 评分：18 | 作者：Jianshun Zhang, Xun Deng, Fang Wang, Jiaxin Ou
  为什么重要：有系统实现，关注吞吐，涉及解耦式架构
  摘要判断：这篇工作主要落在 存储引擎。更偏系统实现。
- [LiBox: A Learned Index as an Array to Minimize Last-Mile Search](https://www.vldb.org/pvldb/volumes/19/contributions)
  来源：PVLDB | 日期：2026-01-01 | 评分：17 | 作者：Jian Zhou, Luna Wang, Shuaihua Zhao, Chen Zhong
  为什么重要：有实验评估，涉及连接处理
  摘要判断：这篇工作主要落在 存储引擎、查询优化。摘要里有较强实验评估信号。
- [DBAIOps: A Reasoning LLM-Enhanced Database Operation and Maintenance System using Knowledge Graphs](https://www.vldb.org/pvldb/volumes/19/contributions)
...
- [SpecDB: LLM-Generated Customized Databases via Feature-Oriented Decomposition](https://arxiv.org/pdf/2605.31097v1.pdf)
  ...
  为什么重要：有系统实现，带基准测试，带生产环境信号，关注延迟
  摘要判断：这篇工作主要落在 存储引擎、查询优化、OLAP / 分析执行。更偏系统实现。
- [Sample-based Distinct Cardinality Estimation for Multiple Attributes in Multi-Dataset Queries](https://www.vldb.org/pvldb/volumes/19/contributions)
...
- [O3-LSM: Maximizing Disaggregated LSM Write Performance via Three-Layer Offloading](https://dl.acm.org/doi/pdf/10.1145/3802093)
  ...
  为什么重要：有实验评估，关注吞吐，关注延迟，涉及连接处理
  摘要判断：这篇工作主要落在 存储引擎、查询优化。摘要里有较强实验评估信号。
- [Robust Predicate Transfer with Dynamic Execution](https://www.vldb.org/pvldb/volumes/19/contributions)
...
- [Efficient Methods for Accurate Sparse Trajectory Recovery and Map Matching.](https://arxiv.org/pdf/2508.10460)
  ...
  为什么重要：有系统实现，有实验评估，贴近真实场景
  摘要判断：这篇工作主要落在 存储引擎、查询优化。更偏系统实现。
- [Abacus: A Cost-Based Optimizer for Semantic Operator Systems](https://www.vldb.org/pvldb/volumes/19/contributions)
...
- [MUFASA: Fast and Accurate Multivariate Time-Series Clustering](https://dl.acm.org/doi/pdf/10.1145/3802090)
  ...
  为什么重要：带基准测试，有实验评估，涉及连接处理
  摘要判断：这篇工作主要落在 查询优化、OLAP / 分析执行。摘要里有较强实验评估信号。
- [One Ring to Shuffle Them All: Scalable Intra-Process Data Redistribution with Ring-Buffer Shuffle in Redpanda Oxla](https://arxiv.org/pdf/2605.29099v1.pdf)
  来源：arXiv cs.DB | 日期：2026-05-27 | 评分：19 | 作者：Adam Szymański, Tyler Akidau
...
- [Deep Research is the New Analytics System: Towards Building the Runtime for AI-Driven Analytics.](https://arxiv.org/pdf/2509.02751)
  ...
  为什么重要：有系统实现，有原型实现
  摘要判断：这篇工作主要落在 查询优化、OLAP / 分析执行。更偏系统实现。
- [TurboLynx: Schemaless Graph Engine Strikes Back for General-Purpose Analytics](https://www.vldb.org/pvldb/volumes/19/contributions)
